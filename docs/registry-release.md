...
```bash
datapan catalog diff --old previous/data-go-kr.registry.json --new data/data-go-kr.registry.json --output .datapan/catalog-diff.json --json
```

The catalog diff records added, removed, and changed datasets by stable list ID.
It is primarily an update-review artifact rather than a mandatory release file,
but the schema is shipped with releases so automation can produce consistent
change logs before a registry snapshot is published.
...
registry JSON, provider index, schema files, catalog audit, error catalog, and
