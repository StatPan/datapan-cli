...
`reports/latest-verification-summary.json`.
Every draft also writes `reports/catalog-audit.json`, `schemas/index.json`, and
`manifest.json` with artifact paths, byte sizes, and SHA-256 checksums.
...
registry JSON, schema files, catalog audit and provider backlog generated from
that registry, optional verification evidence, provenance notes, and a
manifest.
