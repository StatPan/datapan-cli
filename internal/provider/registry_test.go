...
func TestDefaultRegistryIncludesExternalAdapters(t *testing.T) {
	...
	report := registry.IndexReport("2026-06-24T00:00:00Z", "test")
	if report.AdapterCount != 11 || report.HostCount != 14 {
	...
	if report.SplitReadiness.Status != "ready" || report.SplitReadiness.AdapterCount != 11 || report.SplitReadiness.VerificationCapableAdapters != 11 || report.SplitReadiness.CallCapableAdapters != 6 {
		t.Fatalf("unexpected split readiness: %#v", report.SplitReadiness)
	}
	if len(report.SplitReadiness.Reasons) != 0 {
		t.Fatalf("unexpected split readiness reasons: %#v", report.SplitReadiness.Reasons)
	}
	if len(report.Adapters) != 11 || report.Adapters[0].Name != "airport" || report.Adapters[1].Name != "ekape" || report.Adapters[2].Name != "epost" || report.Adapters[3].Name != "folk" || report.Adapters[4].Name != "forest" || report.Adapters[5].Name != "geoje" || report.Adapters[6].Name != "jeonju" || report.Adapters[7].Name != "q-net" || report.Adapters[8].Name != "sisul" || report.Adapters[9].Name != "uiryeong" || report.Adapters[10].Name != "ulsan" {
		t.Fatalf("unexpected provider index adapter: %#v", report)
	}
	if report.Adapters[0].Status != "registered" || report.Adapters[1].Status != "registered" || report.Adapters[2].Status != "registered" || report.Adapters[3].Status != "registered" || report.Adapters[4].Status != "registered" || report.Adapters[5].Status != "registered" || report.Adapters[6].Status != "registered" || report.Adapters[7].Status != "registered" || report.Adapters[8].Status != "registered" || report.Adapters[9].Status != "registered" || report.Adapters[10].Status != "registered" {
	...
	if strings.Join(report.Adapters[8].Hosts, ",") != "data.sisul.or.kr" {
		t.Fatalf("unexpected sisul provider index hosts: %#v", report.Adapters[8].Hosts)
	}
	if strings.Join(report.Adapters[8].Capabilities, ",") != "call,verification" {
		t.Fatalf("unexpected sisul provider index capabilities: %#v", report.Adapters[8].Capabilities)
	}
	if strings.Join(report.Adapters[9].Hosts, ",") != "data.uiryeong.go.kr" {
		t.Fatalf("unexpected uiryeong provider index hosts: %#v", report.Adapters[9].Hosts)
	}
	if strings.Join(report.Adapters[9].Capabilities, ",") != "call,verification" {
		t.Fatalf("unexpected uiryeong provider index capabilities: %#v", report.Adapters[9].Capabilities)
	}
	if strings.Join(report.Adapters[10].Hosts, ",") != "openapi.its.ulsan.kr" {
		t.Fatalf("unexpected ulsan provider index hosts: %#v", report.Adapters[10].Hosts)
	}
	if strings.Join(report.Adapters[10].Capabilities, ",") != "call,verification" {
		t.Fatalf("unexpected ulsan provider index capabilities: %#v", report.Adapters[10].Capabilities)
	}
}
