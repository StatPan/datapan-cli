...
func TestDefaultRegistryIncludesExternalAdapters(t *testing.T) {
	...
	for host, name := range map[string]string{
		...
		"aida.kisti.re.kr":                "kisti",
		"scienceon.kisti.re.kr":           "kisti",
		...
		"www.openfiscaldata.go.kr":        "openfiscaldata",
	...
	report := registry.IndexReport("2026-06-24T00:00:00Z", "test")
	if report.AdapterCount != 138 || report.HostCount != 173 {
	...
	if report.SplitReadiness.Status != "ready" || report.SplitReadiness.AdapterCount != 138 || report.SplitReadiness.VerificationCapableAdapters != 138 || report.SplitReadiness.CallCapableAdapters != 23 {
		t.Fatalf("unexpected split readiness: %#v", report.SplitReadiness)
	}
	...
	expected := []struct {
		...
		{"atfis", "www.atfis.or.kr", "verification"},
		...
		{"ecos", "ecos.bok.or.kr", "verification"},
		...
		{"gogung", "www.gogung.go.kr", "verification"},
		...
		{"koagi", "seedpedia.koagi.or.kr", "verification"},
		...
		{"koroad", "opendata.koroad.or.kr", "verification"},
		...
		{"nrf", "www.kci.go.kr,www.krm.or.kr", "verification"},
		...
		{"openfiscaldata", "www.openfiscaldata.go.kr", "verification"},
		...
		{"tashu", "bike.tashu.or.kr", "verification"},
		...
		{"yuseong", "www.yuseong.go.kr", "verification"},
	}
