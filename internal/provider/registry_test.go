...
func TestDefaultRegistryIncludesExternalAdapters(t *testing.T) {
	...
	report := registry.IndexReport("2026-06-24T00:00:00Z", "test")
	if report.AdapterCount != 7 || report.HostCount != 10 {
	...
	if report.SplitReadiness.Status != "ready" || report.SplitReadiness.AdapterCount != 7 || report.SplitReadiness.VerificationCapableAdapters != 7 || report.SplitReadiness.CallCapableAdapters != 2 {
		t.Fatalf("unexpected split readiness: %#v", report.SplitReadiness)
	}
	if len(report.SplitReadiness.Reasons) != 0 {
		t.Fatalf("unexpected split readiness reasons: %#v", report.SplitReadiness.Reasons)
	}
	if len(report.Adapters) != 7 || report.Adapters[0].Name != "airport" || report.Adapters[1].Name != "ekape" || report.Adapters[2].Name != "epost" || report.Adapters[3].Name != "folk" || report.Adapters[4].Name != "forest" || report.Adapters[5].Name != "jeonju" || report.Adapters[6].Name != "q-net" {
		t.Fatalf("unexpected provider index adapter: %#v", report)
	}
	if report.Adapters[0].Status != "registered" || report.Adapters[1].Status != "registered" || report.Adapters[2].Status != "registered" || report.Adapters[3].Status != "registered" || report.Adapters[4].Status != "registered" || report.Adapters[5].Status != "registered" || report.Adapters[6].Status != "registered" {
	...
	if strings.Join(report.Adapters[5].Hosts, ",") != "openapi.jeonju.go.kr" {
		t.Fatalf("unexpected jeonju provider index hosts: %#v", report.Adapters[5].Hosts)
	}
	if strings.Join(report.Adapters[5].Capabilities, ",") != "verification" {
		t.Fatalf("unexpected jeonju provider index capabilities: %#v", report.Adapters[5].Capabilities)
	}
	if strings.Join(report.Adapters[6].Hosts, ",") != "c.q-net.or.kr,open.api.q-net.or.kr,openapi.q-net.or.kr" {
		t.Fatalf("unexpected q-net provider index hosts: %#v", report.Adapters[6].Hosts)
	}
}
