...
type ForestAdapter struct {
	StaticHostMatcher
}

func NewForestAdapter() ForestAdapter {
	return ForestAdapter{StaticHostMatcher{Hosts: ForestHosts()}}
}

func ForestHosts() []string {
	return []string{
		"api.forest.go.kr",
	}
}

func (a ForestAdapter) Name() string { return "forest" }
...
func (a ForestAdapter) Verify(ctx context.Context, req VerificationRequest) datago.VerificationResult {
	params, missing := forestVerificationParams(req.Params, req.MissingParams)
	...
	if len(missing) > 0 {
		result.Status = "skipped"
		result.Reason = "forest_missing_required_params"
		return result
	}
	...
	result.BodyShape = forestBodyShape(body)
	...
	result.Status = "failed"
	result.Reason = forestFailureReason(providerStatus, message)
	return result
}

func forestRequestURL(endpoint string, params map[string]string, key string) (providerRequestPlan, error) {
...
func forestVerificationParams(params map[string]string, missing []string) (map[string]string, []string) {
	...
	for _, name := range missing {
		if value, ok := forestSafeDefault(name); ok {
			out[name] = value
			continue
		}
...
func forestSafeDefault(name string) (string, bool) {
	switch normalizeParamName(name) {
	case "searchwrd", "search_wrd":
		return "소나무", true
	case "searchmtnm", "search_mt_nm":
		return "북한산", true
	case "searcharnm", "search_ar_nm":
		return "서울", true
...
func forestBodyShape(body []byte) string {
...
func forestFailureReason(status *datago.ProviderStatus, message string) string {
	if status != nil {
		if status.ReasonCode != "" {
			return "forest_" + normalizeReasonCode(status.ReasonCode)
		}
		if status.Code != "" {
			if code := forestMessageReason(status.Message); code != "" {
				return code
			}
			return "forest_result_code_" + normalizeReasonCode(status.Code)
		}
		if code := forestMessageReason(status.Message); code != "" {
			return code
		}
		if code := forestMessageReason(status.AuthMessage); code != "" {
			return code
		}
		if code := forestMessageReason(status.ErrorMessage); code != "" {
			return code
		}
	}
	if code := forestMessageReason(message); code != "" {
		return code
	}
	if strings.TrimSpace(message) != "" {
		return message
	}
	return "forest_provider_error"
}

func forestMessageReason(message string) string {
	upper := strings.ToUpper(strings.TrimSpace(message))
	switch {
	case strings.Contains(upper, "SERVICE KEY IS NOT REGISTERED"):
		return "forest_service_key_not_registered"
	case strings.Contains(upper, "SERVICE KEY"):
		return "forest_service_key_error"
	default:
		return ""
	}
}
