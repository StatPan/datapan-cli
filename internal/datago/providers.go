...
func providerNameForHost(host string) string {
	...
	case host == "apis.data.go.kr" || host == "api.odcloud.kr" || strings.HasSuffix(host, ".data.go.kr"):
		return "data.go.kr"
	case strings.Contains(host, "andong.go.kr"):
		return "andong"
	case strings.Contains(host, "airport.co.kr"):
		return "airport"
	case strings.Contains(host, "calspia.go.kr"):
		return "calspia"
	case host == "cancer.go.kr":
		return "cancer"
	case host == "car.go.kr":
		return "car"
	case strings.Contains(host, "car365.go.kr"):
		return "car365"
	case strings.Contains(host, "codil.or.kr"):
		return "codil"
	case strings.Contains(host, "q-net.or.kr"):
		return "q-net"
	case strings.Contains(host, "worldjob.or.kr"):
		return "worldjob"
	case strings.Contains(host, "safetydata.go.kr"):
		return "safetydata"
	case strings.Contains(host, "ws.bus.go.kr"):
		return "seoul-bus"
	case strings.Contains(host, "www.sisul.or.kr"):
		return "sisul-www"
	case strings.Contains(host, "sisul.or.kr"):
		return "sisul"
	case strings.Contains(host, "uiryeong.go.kr"):
		return "uiryeong"
	case strings.Contains(host, "its.ulsan.kr"):
		return "ulsan"
	case host == "www.ulsan.go.kr":
		return "ulsan-www"
	case strings.Contains(host, "epost.go.kr"):
		return "epost"
	case strings.Contains(host, "emuseum.go.kr"):
		return "emuseum"
	case strings.Contains(host, "ekape.or.kr"):
		return "ekape"
	case strings.Contains(host, "forest.go.kr"):
		return "forest"
	case strings.Contains(host, "gblib.or.kr"):
		return "gblib"
	case strings.Contains(host, "folkency.nfm.go.kr"):
		return "folk"
	case strings.Contains(host, "geoje.go.kr"):
		return "geoje"
	case strings.Contains(host, "gicoms.go.kr"):
		return "gicoms"
	case strings.Contains(host, "gimhae.go.kr"):
		return "gimhae"
	case strings.Contains(host, "gwangjin.go.kr"):
		return "gwangjin"
	case strings.Contains(host, "happysd.or.kr"):
		return "happysd"
	...
	case strings.Contains(host, "itfind.or.kr"):
		return "itfind"
	case strings.Contains(host, "air.jeju.go.kr"):
		return "jeju-air"
	...
	case strings.Contains(host, "kistep.re.kr"):
		return "kistep"
	case strings.Contains(host, "kofpi.or.kr"):
		return "kofpi"
	case strings.Contains(host, "data.dongjak.go.kr"):
		return "dongjak"
	case strings.Contains(host, "data.ex.co.kr"):
		return "ex"
	case strings.Contains(host, "consumer.go.kr"):
		return "consumer"
	case strings.Contains(host, "fairdata.go.kr"):
		return "fairdata"
	case strings.Contains(host, "franchise.ftc.go.kr"):
		return "franchise-ftc"
	case strings.Contains(host, "korad.or.kr"):
		return "korad"
	case strings.Contains(host, "kpx.or.kr"):
		return "kpx"
	...
	case strings.Contains(host, "pqis.go.kr"):
		return "pqis"
	case strings.Contains(host, "psis.rda.go.kr"):
		return "psis"
